...
function getEmailLogs() { return emailLogs; }

function addEmailLog(type, to, status, detail = null) {
  emailLogs.unshift({ timestamp: new Date().toISOString(), type, to, status, detail });
  if (emailLogs.length > 100) emailLogs.pop();
}

// ─── STRATEGY DETECTION ───────────────────────────────────────────────────────
// If RESEND_API_KEY is set → use Resend (HTTP/443, works on Render & all hosts).
// Otherwise              → use Nodemailer via Gmail App Password (works locally).
function useResend() {
...
async function sendViaResend(to, subject, html, logType) {
  ...
  if (error) {
    console.error(`❌ [EMAIL] Resend error (${logType}):`, error.message);
    addEmailLog(logType, to, 'error', `Resend: ${error.message}`);
    return false;
  }

  console.log(`\n✅ [EMAIL] ${logType} email sent to ${to} via Resend (id: ${data.id})\n`);
  addEmailLog(logType, to, 'success', `Resend id: ${data.id}`);
  return true;
}

// ─── SEND VIA NODEMAILER (local dev) ─────────────────────────────────────────
async function sendViaNodemailer(to, subject, html, logType) {
  const user = process.env.EMAIL_USER;
  const pass = process.env.EMAIL_PASS;

  if (!user || !pass) {
    console.warn('⚠️  [EMAIL] Missing EMAIL_USER or EMAIL_PASS.');
    addEmailLog(logType, to, 'warning', 'Missing EMAIL_USER or EMAIL_PASS.');
    return false;
  }

  const transporter = nodemailer.createTransport({ service: 'gmail', auth: { user, pass } });
  const info = await transporter.sendMail({
    from: `"ChooseMyLab" <${user}>`,
    to,
    subject,
    html,
  });

  console.log(`\n✅ [EMAIL] ${logType} email sent to ${to} via Nodemailer (id: ${info.messageId})\n`);
  addEmailLog(logType, to, 'success', `Nodemailer id: ${info.messageId}`);
  return true;
}
...
async function sendWelcomeEmail(userEmail, userName) {
  try {
    ...
    if (useResend()) return await sendViaResend(userEmail, subject, html, 'welcome');
    return await sendViaNodemailer(userEmail, subject, html, 'welcome');
  } catch (error) {
    console.error('❌ [EMAIL] sendWelcomeEmail exception:', error.message);
    addEmailLog('welcome', userEmail, 'error', error.message);
    return false;
  }
}
...
async function sendOtpEmail(userEmail, otpCode) {
  try {
    ...
    if (useResend()) return await sendViaResend(userEmail, subject, html, 'otp');
    return await sendViaNodemailer(userEmail, subject, html, 'otp');
  } catch (error) {
    console.error('❌ [EMAIL] sendOtpEmail exception:', error.message);
    addEmailLog('otp', userEmail, 'error', error.message);
    return false;
  }
}

module.exports = { sendWelcomeEmail, sendOtpEmail, getEmailLogs };
