...
const PopularTests = () => {
  ...
  useEffect(() => {
    const fetchPopularTests = async () => {
      try {
        setLoading(true);
        const response = await api.get('/tests/popular?limit=10');
        ...
        setData([
          ...
          { name: 'Vitamin D', count: 65 },
          { name: 'Kidney Panel', count: 54 },
          { name: 'Urine Routine', count: 42 },
          { name: 'Vitamin B12', count: 35 },
          { name: 'Iron Profile', count: 28 },
        ]);
      } finally {
        setLoading(false);
      }
